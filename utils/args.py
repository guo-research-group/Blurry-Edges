...
def get_args(mode, big=False):
    ...
    parser.add_argument('--img_size', type=int, nargs=2, default=[147,147], help='image size')
    parser.add_argument('--big_img_size', type=int, nargs=2, default=[587,587], help='big image size')
    ...
    # local training
        ...
        parser.add_argument('--dynamic_epoch', type=int, default=200, help='key epoch of dynamic scheduling')
    ...
    # global training
        ...
        parser.add_argument('--gamma_color', type=float, nargs=3, default=[1.0,0.1,0.1], help='weight for color error')
        parser.add_argument('--gamma_color_cons', type=float, nargs=3, default=[0.2,0.1,0.05], help='weight for color consistency error')
        parser.add_argument('--gamma_bndry_cons', type=float, nargs=3, default=[0.05,0.05,0.02], help='weight for boundary consistency error')
        parser.add_argument('--gamma_smthns', type=float, nargs=3, default=[0.005,0.1,0.002], help='weight for smothness error')
        parser.add_argument('--gamma_smthns_cons', type=float, nargs=3, default=[0.005,0.1,0.002], help='weight for smothness consistency error')
        parser.add_argument('--gamma_bndry_loc', type=float, nargs=3, default=[0.0001,0.05,0.0001], help='weight for boundary localization error')
        parser.add_argument('--gamma_depth', type=float, nargs=3, default=[0.0001,0.05,0.5], help='weight for depth error')

        parser.add_argument('--dynamic_epoch', type=int, nargs=3, default=[30,100,200], help='key epoch of dynamic scheduling')
